...
const { forEach, has, isEmpty, isObject, mapValues, reduce, replace, startsWith, uniq } = lodash;
import type { SchemaObject } from 'openapi3-ts/oas30';

export const isObjectSchema = (schema: SchemaObject) =>
	schema.type === 'object' || has(schema, 'properties');
export const isArraySchema = (schema: SchemaObject) =>
...
export function patchSchema<T extends object>(schema: any, schemas: Dictionary<ISchema>): T {
	...
	if (isObjectSchema(schema)) {
		const patchedProperties = mapValues(schema.properties, (propSchema) =>
			patchSchema<ISchema>(propSchema, schemas),
		);

		if (isObject(schema.additionalProperties)) {
			const additionalProperties: ISchema = patchSchema<ISchema>(
