...
import {
	IJSONInputWithSchemaProps,
	JSONSchemaInput,
} from '@/components/monaco-editor/JSONSchemaInput';
	...
	if (schema.enum) {
		...
		return (
			<Select
				{...commonProps}
				allowClear
				options={enumToOptions(schema.enum, enumMap)}
...
function TimeInput({
	...
	isUnix,
	...commonProps
}: IJSONInputWithSchemaProps & { isUnix?: boolean }) {
	const placeholder = displayType(schema) + displayValidate(schema) + displayDefault(schema);
	...
	return (
		<DatePicker
			...
			value={
				commonProps.value
					? isUnix
						? dayjs.unix(commonProps.value)
						: dayjs(commonProps.value)
					: undefined
			}
			showTime={{
				showNow: true,
...
export const RequestParameterInput = ({
	...
	const fieldDesc = (
		<Description
			isBreakWord
			ishighLightDesc
			desc={parameter.description || schema.description || ''}
		/>
	);
							...
							<PatchInput
								...
								onChange={(nextValue) => {
									commonProps.onChange(
										map(commonProps.value, (val, idx) => (index === idx ? nextValue : val)),
									);
								}}
							/>
						</span>
